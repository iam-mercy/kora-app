...
impl PetChainContract {
    ...
    pub fn register_pet(
        ...
        let pet = Pet {
            id: pet_id,
            owner: owner.clone(),
            privacy_level: privacy_level.clone(), // Set privacy_level
        ...
        env.events().publish(
            (String::from_str(&env, "PET_REGISTERED"),),
            PetRegisteredEvent {
                pet_id,
                owner: owner.clone(),
                species: species,
    ...
    pub fn accept_pet_transfer(env: Env, id: u64) {
            ...
            .get::<DataKey, Pet>(&DataKey::Pet(id))
        {
            ...
            if old_owner != new_owner {
                ...
                // EMIT EVENT: PetTransferred
                env.events().publish(
                    (String::from_str(&env, "PET_TRANSFERRED"),),
                    PetTransferredEvent {
                        pet_id: id,
    ...
    pub fn batch_accept_pet_transfers(env: Env, new_owner: Address, pet_ids: Vec<u64>) {
        ...
        for pet_id in pet_ids.iter() {
            ...
            if old_owner != new_owner {
                ...
                // EMIT EVENT: PetTransferred (per pet)
                env.events().publish(
                    (String::from_str(&env, "PET_TRANSFERRED"),),
                    PetTransferredEvent {
                        pet_id: pet_id,
    ...
    pub fn add_vaccination(
        ...
        env.events().publish(
            (String::from_str(&env, "VACCINATION_ADDED"),),
