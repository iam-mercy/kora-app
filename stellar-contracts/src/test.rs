...
// ============== VACCINATION TRACKING SYSTEM TESTS ==============
...
fn test_multiple_record_and_get_vaccination() {
    ...
    let administered_time_2 = 2735689600;
    let next_due_date_2 = administered_time_2 + 31536000;
    let now = env.ledger().timestamp();
    ...
    assert_eq!(record_2.administered_at, administered_time_2);
    assert_eq!(record_2.next_due_date, next_due_date_2);
...
fn test_get_upcoming_vaccinations() {
    ...
    // The one due in 10 days should appear
...
fn test_is_vaccination_current() {
    ...
    let future_due = base_time + 31536000;
    let past_due = 1000000u64;
    ...
    // Leukemia was never given, so should return false
    assert_eq!(
...
fn test_get_overdue_vaccinations() {
    ...
    // Add vaccination with overdue date
    let overdue_date_1 = 100000u64;
    client.add_vaccination(
        &pet_id,
        &vet,
        &VaccineType::Rabies,
    ...
    // Add another overdue vaccination
    let overdue_date_2 = 200000u64;
    client.add_vaccination(
        &pet_id,
        &vet,
        &VaccineType::Bordetella,
    ...
    // Add vaccination with far future due date (not overdue)
    ...
    // At timestamp 0, nothing is overdue yet
...
fn test_tamper_proof_vaccinations() {
    ...
    // Retrieve again - should be identical (tamper-proof)
...
fn test_multiple_vaccinations_same_type() {
    ...
    // Add old Rabies vaccination (expired)
    client.add_vaccination(
        &pet_id,
        &vet,
        &VaccineType::Rabies,
    ...
    // Latest rabies vaccination should be current
    assert_eq!(
        client.is_vaccination_current(&pet_id, &VaccineType::Rabies),
        true
    );
...
fn test_vaccination_for_nonexistent_pet() {
    ...
    // Just verify setup works - actual vaccination attempt would panic
...
fn test_vaccination_for_nonexistent_pet_returns_empty() {
    ...
    // Query vaccination history for non-existent pet
    let history = client.get_vaccination_history(&999);
...
fn test_vaccination_status_overdue() {
    ...
    // Vaccination that would be overdue at any time > 0
    // Since ledger timestamp starts at 0, we need past_due to be 0 as well
    // to make it already overdue
    let past_due = 0u64;
    ...
    // At timestamp 0, due date 0 means not current (due_date > current_time is false)
...
fn test_upcoming_vaccinations_within_threshold() {
    ...
    let upcoming = contract.get_upcoming_vaccinations(&pet_id, &60);
...
fn test_get_overdue_vaccinations_comprehensive() {
    ...
    // At timestamp 0, due date 0 is not overdue (< is false when equal)
    // So this vaccination won't show as overdue at initial timestamp
    let past_due_date = 0u64;
    ...
    // At timestamp 0, due_date 0 is not < 0, so not overdue
    assert!(overdue.len() == 0);
}
