...
| Wave 9 # | Title | Category | Priority |
|:---:|---|:---:|:---:|
| #1 | Resolve variable shadowing, duplicate declarations, and inverted state emission in `correctMedicalRecord` | Celo/Solidity | P0 |
| #2 | Enforce `whenNotPaused` modifier on `correctMedicalRecord` in `KoraRegistry` | Celo/Solidity | P1 |
| #3 | Implement two-step ownership transfer (`Ownable2Step`) for admin role in `KoraRegistry` | Celo/Solidity | P2 |
| #4 | Enforce recipient consent pattern for `transferPet` to prevent forced pet transfers | Celo/Solidity | P1 |
| #5 | Deduplicate redundant `correctMedicalRecord` test suites in Celo Hardhat tests | Testing | P3 |
| #6 | Upgrade `pet-transfer-adoption` contract to Soroban SDK v28 and resolve transitive `ed25519-dalek` compilation failure | Stellar/Soroban | P0 |
| #7 | Integrate `pet-transfer-adoption` into root workspace `Cargo.toml` and CI test matrices | CI/CD | P1 |
| #8 | Implement systematic TTL extension strategy across all persistent and instance storage | Stellar/Soroban | P0 |
| #9 | Migrate unbounded pet records and audit logs from instance storage to persistent storage | Architecture | P0 |
| #10 | Implement `decrement_pet_storage` to restore user quota upon record purging and photo removal | Data Integrity | P1 |
| #11 | Replace O(N*M) quadratic iteration in `compact_storage` with single-pass compaction | Performance | P2 |
| #12 | Restrict public unauthenticated medical record queries to pet owners and authorized veterinarians | Access Control | P0 |
| #13 | Enforce caller authentication on `add_activity_record` | Access Control | P0 |
| #14 | Enforce caller authorization and breed registry permissions on `set_pet_traits` | Access Control | P1 |
| #15 | Validate sire and dam existence, gender compatibility, and caller identity in `add_breeding_record` | Smart Contract | P1 |
| #16 | Fix COI (Coefficient of Inbreeding) calculation to account for parent-child inbreeding | Data Integrity | P2 |
| #17 | Make `ActivityKey::IdempotencyWindow` configurable and implement `purge_expired_idempotency_keys` | Smart Contract | P2 |
| #18 | Enforce `caller.require_auth()` in `get_emergency_info_with_reason` and `notify_emergency_contacts` to prevent audit forgery | Security | P0 |
| #19 | Replace static public-key pseudo-encryption in `get_encryption_key` with secure user-derived key management | Security | P0 |
| #20 | Enforce upgrade proposal timelock delay and multi-admin quorum in `execute_upgrade` | Upgradeability | P0 |
| #21 | Correct `PreviousWasmHash` storage in `execute_upgrade` to prevent corrupted upgrade rollbacks | Upgradeability | P1 |
| #22 | Prevent single-admin denial of service on multisig threshold proposals | Security | P1 |
| #23 | Require multisig proposal threshold to modify `set_quorum_percent` and prevent zero-quorum bypass | Access Control | P1 |
| #24 | Deprecate unauthenticated `propose_init` and resolve competing contract initialization entrypoints | Security | P1 |
| #25 | Fix `new_owner` initialization in `register_pet` and implement pet transfer cancellation in `KoraContract` | Smart Contract | P1 |
| #26 | Fix caller resolution in `get_pet_age`, `get_pet_age_with_lifespan`, and `get_pet_by_tag` for non-public pets | Smart Contract | P1 |
| #27 | Synchronize error codes between `ContractError`, `KoraError`, and runtime error registry | Data Integrity | P1 |
| #28 | Replace full linear scan in `matching_subscription_ids` with indexed subscription mapping | Performance | P2 |
| #29 | Fix pagination offset boundary check in `get_lab_results` | Smart Contract | P2 |
| #30 | Re-link and fix 49 orphaned test modules in `stellar-contracts/src/` into the crate test harness | Testing | P1 |
| #31 | Deduplicate redundant CI workflows between `stellar-contracts.yml` and `stellar.yml` | CI/CD | P2 |
| #32 | Cache `stellar-cli` installation in GitHub Actions workflows to eliminate redundant compilation | CI/CD | P2 |
| #33 | Fix unsound auto-merge rule that bypasses pull request review approval in `auto-merge.yml` | CI/CD | P1 |
| #34 | Add test and build steps for `pet-transfer-adoption` contract to CI pipeline | CI/CD | P2 |
| #35 | Fix permission scopes and branch triggers in `pr-review-bot.yml` | CI/CD | P2 |
| #36 | Synchronize `docs/error-codes.md` with on-chain `ContractError` enum variants | Documentation | P2 |
| #37 | Update `docs/api.md` and `docs/openapi.yaml` to document missing medical record privacy and authorization parameters | Documentation | P2 |
| #38 | Update `docs/architecture.md` to document dual-chain differences between Stellar and Celo implementations | Documentation | P2 |
| #39 | Correct contract initialization and multisig onboarding guides in `docs/development.md` | Documentation | P2 |
| #40 | Align OpenAPI schema response structures for pet transfer and adoption records | API/Backend | P2 |
| #41 | Prevent duplicate pet microchip registration across different owners | Data Integrity | P1 |
| #42 | Add expiration timestamp check to veterinarian license verification status | Smart Contract | P2 |
| #43 | Implement insurance policy registration and claim payout status transition | Smart Contract | P1 |
| #44 | Validate dosage, frequency, and prescribing veterinarian in `Medication` records | Smart Contract | P2 |
| #45 | Implement caller authorization and privacy checks for pet photo queries and deletion | Access Control | P1 |
| #46 | Prevent deactivated or unverified veterinarians from adding medical and vaccination records | Access Control | P1 |
| #47 | Validate diet plan calorie targets and macronutrient percentage totals | Data Integrity | P2 |
| #48 | Prevent re-initialization of insurance policy parameters and duplicate claim submissions | Security | P1 |
| #49 | Add integration test suite for cross-contract calls between `KoraContract` and `pet-transfer-adoption` | Testing | P2 |
| #50 | Clean up unused dependencies and redundant profile configurations in `stellar-contracts` | Technical Debt | P3 |
...
### 1. Important Architectural Observations
- **Storage Footprint & Imminent Protocol Exhaustion**: The single most dangerous architectural risk identified in `stellar-contracts/src/lib.rs` is the near-total reliance on `env.storage().instance()` for storing unbounded entities (`Pet`, `MedicalRecord`, `DietPlan`, `ActivityKey`, `Dispute`). On Soroban, instance storage is capped at protocol limits (~128 KiB) and loads all keys into VM memory during every invocation. Migrating these unbounded collections to `persistent()` storage (Issue #9) and implementing systematic TTL extension (Issue #8) must precede production scaling.
- **Illusory Cryptography & Key Management Risks**: The cryptographic obfuscation implemented in `get_encryption_key` (Issue #19) provides zero security against network observers because the symmetric key is derived purely from public blockchain data (the contract address and the first admin address). Worse, rotating administrators will alter the derived key, permanently rendering all previously encrypted pet names, birthdays, and medical alerts unrecoverable. Transitioning to genuine client-side envelope encryption or static salt management is critical.
- **On-Chain Contract Size Constraints**: The compiled and optimized Wasm bytecode for `KoraContract` exceeds 214 KiB, surpassing Soroban's network limit of 128 KiB (`contract_max_size_bytes`). The contract cannot currently be deployed to any live Stellar network without decomposition into specialized peripheral contracts.

### 2. Significant Inconsistencies Discovered
- **Error Code Mappings**: Serious discrepancies exist between `ContractError` enum discriminants in `stellar-contracts/src/lib.rs`, `KoraError`, the on-chain multi-language error registry in `initialize_error_messages`, and `docs/error-codes.md` (Issues #27, #36). For instance, discriminant 1 is `AdminAlreadyApproved` in the contract, but documented as `Unauthorized` in `error-codes.md`.
- **Cross-Chain Architecture Disparities**: The Celo implementation (`celo-contracts/contracts/KoraRegistry.sol`) lags significantly behind the Stellar contract in both features and security architecture. While Stellar incorporates two-step pet transfers, field-level encryption, role separation, and multisig governance, Celo uses single-step transfers, plaintext storage, and a single admin. In addition, `KoraRegistry.sol` contains severe code defects, including duplicate mappings, conflicting event declarations, and inverted state logging in `correctMedicalRecord` (Issues #1, #2, #3, #4).
- **Initialization & Threshold Conflicts**: The codebase contains competing contract initialization paths (`init_admin`, `init_multisig`, and unauthenticated `propose_init`), as well as contradictory threshold modification mechanisms (full-quorum `set_threshold` vs standard-quorum `propose_action(ParameterChange(AdminThreshold))` and unilateral `set_quorum_percent(0)`).

### 3. Notable Areas Already Well Covered
- **Pet Validation & Constraints**: Basic pet validation (`validate_pet_name`, `validate_breed`) and birthday timestamp parsing are thoroughly guarded with unit tests and property-based fuzz tests in `stellar-contracts/src/test_proptest_medical.rs` and `PROPTEST_MEDICAL_RECORDS.md`.
- **Activity Streak Milestone Caps**: Streak updates in `add_activity_record` enforce bounds with `MAX_MILESTONES = 32` and deduplicate milestones against `STREAK_MILESTONE_DAYS`, backed by rigorous test cases in `stellar-contracts/src/test_activity.rs`.
- **Custody Chain Integrity Verification**: The custody history ledger uses cryptographic SHA-256 hash chaining (`verify_custody_chain`) to detect historical record tampering, with comprehensive test coverage in `stellar-contracts/src/test_custody_chain.rs`.
- **Deterministic Test Fixtures**: Shared test fixtures (`stellar-contracts/src/test_fixtures.rs`) establish reproducible timestamps, multisig signers, and sample pet identities across tests.

### 4. Areas Where Evidence Was Insufficient to Confidently Create an Issue
- **Off-Chain IPFS Gateway SLA & Pinning**: While IPFS hash format validation (`validate_ipfs_hash`) was inspected on-chain, external pinning service configurations (such as Pinata or Web3.Storage API keys and retention policies) reside outside this repository.
- **Push Notification Service Architecture**: The contract emits event subscriptions and notification rate limits, but the backend relayer daemon responsible for dispatching mobile APNS/FCM notifications is not part of this monorepo.
- **Cross-Chain Bridge Relayer Implementation**: While cross-chain identity tests exist (`test_cross_chain_identity.rs`), the operational bridge relayer or cross-chain message passing protocol between Celo and Stellar is not defined in the contract sources.

### 5. Assumptions Deliberately Avoided
- **Did Not Assume EVM/Soroban 1:1 Feature Parity**: Did not treat differences between EVM and Soroban runtime models (e.g., storage models, gas meters, event indexing) as bugs unless they represented broken user requirements or security oversights.
- **Did Not Assume Legacy Single-Admin Mode Was a Defect**: Recognized that single-admin functionality is intentionally maintained alongside multisig governance to support initial bootstrapping before multisig onboarding.
- **Did Not Conflate Test File Count With Coverage**: Avoided assuming that the presence of 71 test files in `stellar-contracts/src/` implied passing test execution without verifying crate module declarations in `lib.rs`.
